...
- **`graylog_auth_backend` and `graylog_auth_backend_activation`**: new resources for authentication service backends (LDAP and Active Directory) via `/system/authentication/services/{backends,configuration}`. The existing `graylog_ldap_setting` targets the pre-4.0 `/system/ldap/settings` endpoint, which modern Graylog no longer serves, so directory authentication previously had to be configured by hand or by an out-of-band script. `graylog_ldap_setting` is now deprecated in favour of these; it still works and is not scheduled for removal.
  ...
  **`system_user_password` is a top-level attribute, not part of `config_json`.** Graylog stores it and reports back `{"is_set": true}` rather than the value, so it can never be refreshed — modelling it as its own `Optional`+`Sensitive` attribute puts that in the schema instead of hiding it in a JSON blob. It is merged into the API document on create and update, and leaving it unset keeps whatever password is already stored, which is what makes an imported backend manageable. Setting it inside `config_json` is rejected, since two sources of truth for one field would silently disagree, and so is an empty string — omitting the attribute is how you say "keep the stored one".

  Backend configuration is otherwise polymorphic (`ldap` vs `active-directory`), so it stays as `config_json` with the `type` discriminator inside. Server-side additions such as `email_attributes` are projected out of drift detection; on import, where there is no prior document to project against, the server's configuration is adopted whole (minus the password) so that the attribute is usable rather than empty.

  Every single-backend response is wrapped in a `{"backend": {...}}` envelope; decoding it as a bare struct yields an empty backend and no error, so it is unwrapped explicitly. Update sends the ID in the body as well as the path, and reads it from state rather than the plan. The activation endpoint only accepts `POST` — `PUT` answers 405. `default_roles` is a set rather than a list, because Graylog does not guarantee the order it returns them in. Destroying an activation reads the current selection first and only clears it when it is still the one that resource set, so it cannot take down a backend something else activated in the meantime.

- **`graylog_cluster_config`**: new resource wrapping `/system/cluster_config/{class}`, Graylog's store of cluster-wide settings keyed by the fully qualified name of the class that reads them. Many of those settings have no dedicated API, so they previously had to be driven by out-of-band `curl` calls from a `null_resource` provisioner; a single generic resource covers all of them. `class` forces replacement (a different class is a different document) and `Delete` removes the stored document, which returns the class to the default compiled into the server.

  Whole-document semantics, verified live: Graylog deserializes the body into the target class and **rejects incomplete documents** (`Couldn't parse cluster configuration ... Null <field>`), so this is a replace and never a patch. Because the server stores what it is given verbatim, state keeps the practitioner's document rather than the server echo — the failure mode that produced the `graylog_input`/extractors "Update ran on every single apply" bug in v0.5.0. `Read` compares canonical JSON and only adopts the server document when it actually differs, so formatting and key order never cause a diff. An integration test asserts that verbatim round-trip on every supported major version, so a future Graylog that starts materializing defaults into stored documents fails loudly instead of silently producing a permanent diff.

  Not-found handling covers all three server behaviors observed on 5.x/6.x/7.x: `204` for a known class with nothing stored, `404` for a class that cannot be resolved, and `400` for a class outside the server's `safe_classes` allowlist. The first two map to `ErrNotFound` so the resource leaves state cleanly; `Delete` treats an already-absent document as success.
- **`graylog_lookup_cache`, `graylog_lookup_adapter`, `graylog_lookup_table`**: new resources covering `/system/lookup/{caches,adapters,tables}`. Lookup tables are what pipeline rules resolve with `lookup()`/`lookup_value()`, so until now a pipeline rule could be managed by Terraform while the table it depends on had to be clicked together in the UI or bootstrapped by a script — an apply that succeeded while the rule silently enriched nothing.

  Cache and adapter configuration is polymorphic (dozens of types, each with its own field set), so it is expressed as `config_json` with the `type` discriminator inside; the table has a fixed shape and gets typed attributes. Adapter `config_json` is marked sensitive, since adapter configurations routinely carry credentials.

  **Server-side defaults are projected out of drift detection.** Graylog materializes the defaults of the chosen type when it stores a configuration — submitting five keys to a `guava_cache` reads back nine, the extra four being nulls the practitioner never wrote. Comparing the full echo would report a difference on every plan forever, the same class of failure as the v0.5.0 `graylog_input`/extractors bug. `Read` therefore narrows the echo to the keys actually under management before comparing; a managed key that changes server-side is still reported. Acceptance tests assert an empty plan after create and after update.

  **The tables endpoint returns a paginated envelope even when addressed by ID**, unlike the cache and adapter endpoints which return the object directly. Decoding it as a bare object yields a zero-valued struct and no error, so `GetLookupTable` unwraps it explicitly and treats an empty envelope as not-found.

  **Update sends the ID in the body as well as the path** — Graylog rejects a mismatch with `URL parameter does not match parameter in request body` — and reads that ID from state rather than the plan, the bug that produced 405s in v0.3.5 and v0.5.0. The Terraform ID is the Mongo ID rather than the name, so renaming is an in-place update instead of a cascading replace; import accepts either and stores the resolved ID.

  Graylog refuses to delete a cache or adapter that a table still references, so no pre-delete usage scan is needed: referencing resource attributes gives Terraform the ordering, and the server guards the rest.

### Fixed
- **`graylog_input` extractors (#18)**: replaced the remaining troubleshooting example that still used the removed `extractors = jsonencode(...)` shape and the invalid `type` field with the structured `extractor` block and `extractor_type`. Extractors without converter blocks now explicitly default to a non-nil empty converter list before reaching the client; added a wire-format regression test proving create requests contain `"extractor_type"` and `"converters": []` (never `"type"` or `null`).
